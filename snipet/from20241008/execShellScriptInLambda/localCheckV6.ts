...
// Unix時間を JST と UTC の日時に変換する関数（ログ用途のみで使用）
...
const programStartDate = getDateFromUnixTime(programStartUnix.milliseconds);
console.log(`[INFO] Program started at (UTC): ${programStartDate.utc}`);
console.log(`[INFO] Program started at (JST): ${programStartDate.jst}`);
...
function calcUTCTimeSlotsFromUnix(unixTimeMillis: number): { start: Date; end: Date } {
  ...
  // 開始・終了時間を構築 (UTC)
  const start = new Date(unixTimeMillis); // 元の日時から生成
  const end = new Date(unixTimeMillis); // 元の日時から生成
 // 何をしているか:
  // 現在の日時 (date) を基に、新しい Date オブジェクトを作成しています。
  // start と end は、それぞれ 開始時間 と 終了時間 を表します。
  // なぜ新しい Date を作成するか:
  // Date オブジェクトはミュータブル（変更可能）なので、date を直接操作すると元の date に影響を与えてしまいます。
  // 新しいオブジェクトを作ることで、安全に別々の値（start と end）を計算できます。

  start.setUTCHours(startHour, startMinute, 0, 0);
  end.setUTCHours(endHour, endMinute, 59, 999);
  // 何をしているか:
  // endHour と endMinute を基に、end オブジェクトの時刻を設定しています。
  // UTC の時間を基準に設定していることに注意してください。
  // 設定内容:
  // setUTCHours メソッドの引数:
  // endHour: 終了時間の時間 (hour)。
  // endMinute: 終了時間の分 (minute)。
  // 59: 秒を 59 に設定。
  // 999: ミリ秒を 999 に設定。
  // 結果:
  // 例えば、endHour = 14、endMinute = 9 の場合、end は次のように設定されます:
  // 2024-12-18T14:09:59.999Z
...
// 現在の Unix時間を取得してTimeスロットを計算(UTC)
const { start, end } = calcUTCTimeSlotsFromUnix(programStartUnix.milliseconds); // Unix時間を基にスロット計算
...
async function processItems() {
  for (const item of items) {
    ...
    console.log(`[INFO] Start Time (UTC): ${formattedStartTime}, End Time (UTC): ${formattedEndTime}`);
...
// 処理を開始
processItems().then(() => {
  const programEndUnix = getUnixTime();
  console.log(`[INFO] Program ended at (Unix Time): ${programEndUnix.milliseconds} ms`);
  const programEndDate = getDateFromUnixTime(programEndUnix.milliseconds);
  console.log(`[INFO] Program ended at (UTC): ${programEndDate.utc}`);
  console.log(`[INFO] Program ended at (JST): ${programEndDate.jst}`);
});
