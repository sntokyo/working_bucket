...
async function processItems() {
  ...
  const dataDir = path.resolve('jmbsc-get-cloud-data-stack.get.script/data');
  ...
  for (const item of items) {
    ...
    try {
      ...
      // `data` ディレクトリ内のファイルを削除
      const files = await fs.readdir(dataDir); // ディレクトリ内のファイル一覧を取得
      ...
      console.error(`[FAILED] Failed to process item: ${item}`, err);
    }
  }
