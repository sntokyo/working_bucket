...
import { IntegrationStackPipelineStack } from "../lib/integration-stack-pipeline-stack";
...
if (deploymentType === "devops") {
  ...
  new DevopsPipelineStack(app, stackName, {
    description: `This stack is cdk infra pipeline for the account ${account.accountNumber} and region ${account.pipelineRegion}`,
    accountNumber: account.accountNumber,
    region: account.pipelineRegion,
    stackName: stackName,
    environment: account.environment,
    ...
    artifactBucketName: artifactBucketStack.artifactBucket,
  });
}
...
if (deploymentType === "infra" && envConfig !== "ntdev" && envConfig !== "stg") {
  ...
  new IntegrationStackPipelineStack(app, stackName, {
    description: `This stack is cdk infra stack for the account ${account.accountNumber} and region ${account.region}`,
    accountNumber: account.accountNumber,
    region: account.region,
    environment: account.environment,
